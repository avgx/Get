...
@Test func sseAccumulatorEmitsOnBlankLine() {
    ...
    let ev = acc.push("")
    #expect(ev == ServerSentEvent(id: "42", event: "tick", data: "hello"))
}
...
@Test func sseAccumulatorFinishFlushesWithoutTrailingBlankLine() {
    ...
    let ev = acc.finish()
    #expect(ev == ServerSentEvent(data: "tail"))
}
...
@Test func sseAccumulatorTreatsCarriageReturnOnlyLineAsEventBoundary() {
    ...
    #expect(acc.push("\r") == ServerSentEvent(event: "stream-data", data: "one"))
    #expect(acc.push("event: stream-data") == nil)
    #expect(acc.push("data: two") == nil)
    #expect(acc.push("\r") == ServerSentEvent(event: "stream-data", data: "two"))
    #expect(acc.push("event: end-of-stream") == nil)
    #expect(acc.push("data: ") == nil)
    #expect(acc.push("\r") == ServerSentEvent(event: "end-of-stream", data: ""))
}
